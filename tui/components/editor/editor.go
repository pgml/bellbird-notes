...
// ReopenLastClosedBuffer attempts to open the last buffer from
// `editor.ClosedBuffers`
func (editor *Editor) ReopenLastClosedBuffer() message.StatusBarMsg {
	if len(editor.ClosedBuffers) <= 0 {
		return message.StatusBarMsg{}
	}
...
// CloseCurrentBuffer deletes the currently opened editor buffer and appends
// it to the closed buffers list.
func (editor *Editor) CloseCurrentBuffer() message.StatusBarMsg {
	editor.ClosedBuffers = append(editor.ClosedBuffers, editor.CurrentBuffer.path)
	editor.DeleteCurrentBuffer()
	return message.StatusBarMsg{}
}

func (editor *Editor) openLastBufferOrReset() {
	buffers := *editor.Buffers

	if len(buffers) > 0 {
		lastBuffer := buffers[len(buffers)-1]
		editor.OpenBuffer(lastBuffer.path)
	} else {
		editor.reset()
	}
}
